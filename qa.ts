...
while (true) {
  console.log(
    chalk.bold.underline.bgGreenBright(
      `\n\tWelcome TO Student Management System\n\t`
    )
  );

  //for student unique ID number
  const randoNumber: number = Math.floor(10000 + Math.random() * 90000);
  ...
  console.log(
    chalk.underline.red(
      `\nYOUR COURSE FEE IS UNPAID\n\nYOUR CURRENT BALNCE IS: ${chalk.gray(
        myBlance
      )}\n`
    )
  );
  ...
  let paymentType = await inquirer.prompt([
    ...
    },
    {
      ...
      type: "input",
      message: chalk.red(
        `Transfer Money(NOTE ENTER ONLY GIVEN AMOUNT COUSRE FEE!${chalk.yellow(
          program.coursess
        )}):`
      ),
      validate: function (value) {
  ...
  console.log(
    chalk.yellow(
      `\n You Select Payment Method Through ${chalk.underline.bold(
        paymentType.payment
      )}\n`
    )
  );
  ...
  if (TutionFees === paymentAmounts) {
    console.log(
      chalk.bgGreenBright(
        `\nCongrats You Have Sucessfull Enrolled in ${chalk.underline.bold.bgGray(
          program.coursess
    ...
    if (ans.select === "View Status") {
      ...
      console.log(
        chalk.white(`Tution Fee Paid : ${chalk.blue(paymentAmounts)}`)
      );
      console.log(
        chalk.white(`Balance : ${chalk.blue((myBlance += paymentAmounts))}\n`)
      );
